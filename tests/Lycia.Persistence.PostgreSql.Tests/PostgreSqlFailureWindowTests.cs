...
public class PostgreSqlFailureWindowTests(PostgreSqlContainerFixture fixture)
{
    ...
    /// A real PostgreSQL socket-level "commit sent, ack lost" race is not reliably reproducible from a
    ...
    /// this container is shared (via <see cref="PostgreSqlContainerFixture"/>, an <c>ICollectionFixture</c>)
    /// by every other test in the same collection, and restarting it mid-run knocked every concurrently
    /// running test's connection out too (observed: the whole suite failed with connection errors once
    /// this test restarted the shared container). Clearing the ADO.NET connection pool for this
    /// connection string reproduces the same "reconnect after every physical connection is gone"
    /// condition without disturbing the shared container that other tests depend on.
    /// </summary>
    [Fact]
    public async Task SagaStore_reconnects_and_round_trips_after_connection_pool_reset()
    {
        ...
        await PostgreSqlSchemaMigrator.RunAsync(options);
        var storeBeforeReset = new PostgreSqlSagaStore(options, null!, null!, null!, null);
        ...
        var storeAfterReset = new PostgreSqlSagaStore(options, null!, null!, null!, null);
