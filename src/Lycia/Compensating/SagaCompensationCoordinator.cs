    ...
    : ISagaCompensationCoordinator
{
    ...
    public async Task CompensateParentAsync(Guid sagaId, Type stepType, Type handlerType, IMessage message, CancellationToken cancellationToken = default)
    {
        ...
        await EnsureClaimAndAttemptPropagationAsync(sagaId, message.MessageId, message.ParentMessageId, eventBus,
            sagaStore, cancellationToken).ConfigureAwait(false);
    }
        ...
        IEventBus eventBus, ISagaStore sagaStore, CancellationToken cancellationToken)
    {
        ...
        var claim = await sagaStore.EnsureAndClaimCompensationPropagationAsync(sagaId, childMessageId,
        ...
        await AttemptPropagationAsync(sagaId, childMessageId, eventBus, sagaStore, cancellationToken)
            .ConfigureAwait(false);
    }
