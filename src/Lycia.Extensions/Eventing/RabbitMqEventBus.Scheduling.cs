...
public sealed partial class RabbitMqEventBus
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _channel.ExchangeDeclareAsync(finalExchange, finalExchangeType, durable: true, autoDelete: false,
            arguments: null, cancellationToken: cancellationToken).ConfigureAwait(false);
        try
        {
            await _channel.QueueDeclareAsync(queueName, durable: true, exclusive: false, autoDelete: false,
                arguments: arguments, cancellationToken: cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            Persistent = true,
            MessageId = record.MessageId.ToString("D"),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var state = await _channel.QueueDeclarePassiveAsync(resource.CanonicalName, cancellationToken)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await _channel.QueueDeleteAsync(resource.CanonicalName, ifUnused: true, ifEmpty: true,
